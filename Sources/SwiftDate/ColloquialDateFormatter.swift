...
public class ColloquialDateFormatter {
    ...
    public static func colloquial(from fDate: DateInRegion, to tDate: DateInRegion, options fOpt: ColloquialDateFormatter.Options? = nil) -> String? {
        ...
        // Months difference
        if var months = value(for: .month, in: cmps) {
            if isDistant { return distant(for: .month, in: tDate) }
            if(cmps.day != 0) {
                months = abs(months)+1
            }
            return colloquialString(for: .month, value: months, future: isFuture, args: months)
        }
        
        // Days difference
        if var days = value(for: .day, in: cmps) {
            ...
            if days < ColloquialDateFormatter.DAYS_IN_WEEK || !isWeekAllowed {
                ...
                if isWeekAllowed == true { // Weeks representation is allowed
                    let weeks = Int((Double(days) / Double(ColloquialDateFormatter.DAYS_IN_WEEK)).rounded(.up))
        ...
        // Hours difference
        if var hours = value(for: .hour, in: cmps) {
        ...
        // Minutes difference
        if var minutes = value(for: .minute, in: cmps) {
            if isImminentValue(for: .minute, value: minutes) { // is imminent
                return imminentString()
            }
            // standard minutes difference
            if(cmps.second != 0) {
                minutes = abs(minutes)+1
            }
            return colloquialString(for: .minute, value: minutes, future: isFuture, args: minutes)
        }
