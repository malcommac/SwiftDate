...
/**
  This structure is used to encapsulate all attributes you can use to specify the formatting behavior of a string or an interval of time. You need to provide it when you want to convert an `NSDate`, `DateInRegion` or a `NSTimeInterval` instance into a colloquial representation.
*/
public struct FormatterStyle {
	/// Configure the strings to use for unit names such as days, hours, minutes and seconds.
	/// Use this property to specify the style of abbreviations used to identify each time component.
	/// By default the value of this property is .Abbreviated; so for example, one hour and ten minutes is displayed as “1h 10m”.
	public var style :NSDateComponentsFormatterUnitsStyle = .Abbreviated
	
	/// The bitmask of calendrical units such as day and month to include in the output string. By default all available values are
	/// used; this means each unit of time is part of the output string if the relative value is not zero.
	/// You can use:
	///	* `Year`
	///	* `Month`
	///	* `WeekOfMonth`
	...
	/// This property can be used to limit the number of units displayed in the resulting string. For example if set to 2, instead of
	/// “1h 10m, 30s” the output string will be “1h 10m”. You can use it to provide a round up version of the output when you are
	/// constrained.
	public var maxUnits :Int = 0
	
	/// Setting the value of this property to `true` adds phrasing to output strings to reflect that the given time value is approximate and not exact.
	/// Using this property yields more correct phrasing than simply prepending the string “About” to an output string.
	...
	/// When this property is set to `true`, the formatted value would be “63s”. When the value of this property is `false`,
	...
	/**
	Initialize a new `FormatterStyle` struct you can use to format a `NSDate`,`DateInRegion` or `NSTimeInterval`
	
	- parameter style abbreviations you could use to format each time unit of the output string (by default `.Abbreviated` form is used)
	- parameter units units you want to include in output string (by default all values are set)
	- parameter max number of units you wan to include in ouput string (by default is `0` and means: do not omit anything)
	
	- returns: a new `FormatterStyle` struct
	*/
	public init(style :NSDateComponentsFormatterUnitsStyle = .Abbreviated, units :NSCalendarUnit? = nil, max :Int = 0) {
		self.style = style
